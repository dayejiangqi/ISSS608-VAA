...
## 5.1 Profitability vs Risk Metrics

### 5.1.1 Premium vs Incurred
...
### 5.1.2 Loss Ratio vs Claim frequency
...
```{r}
ggplot(portfolio, aes(x = claim_frequency, y = loss_ratio)) +
...
### 5.1.3 Profit vs Claim Severity
...
## 5.2 Policy & Vehicle Characteristics

### 5.2.1 Policy & Vehicle Characteristics
...
ggplot(portfolio, aes(x = fuel_type, y = loss_ratio, fill = fuel_type)) +
...
### 5.2.3 Vehicle Age vs Claim Frequency
...
### 5.4.4 Premium vs Incurred Volatility by Year 

An animated bubble chart template to show Premium vs Incurred volatility by year. 

```{r}
# Animated bubble chart: Premium vs Incurred by Year
gg <- ggplot(portfolio, 
       aes(x = total_premium, 
           y = total_incurred, 
           size = total_exposure,   # bubble size
           colour = policy_type)) + # bubble color
  geom_point(aes(frame = year), alpha = 0.7) +
  scale_size(range = c(2, 12)) +
  labs(
    title = "Premium vs Incurred Volatility by Year",
    x = "Total Premium",
    y = "Total Incurred"
  ) +
  theme_minimal()

# Convert to interactive animated plot
ggplotly(gg)
```

## 5.5 Statistical validation

### 5.5.1 Chi-square tests → categorical vs categorical (e.g., policy_type vs policy_status).

### 5.5.2 t-tests / ANOVA → numeric vs categorical (e.g., total_incurred across fuel_type).

### 5.5.3 Correlation / regression models → numeric vs numeric (e.g., vehicle_value vs total_premium).

To detect relationships among numeric drivers (premium, incurred, claims, profit, loss ratio).
